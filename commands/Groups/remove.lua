...
return {
	name = "remove",
	description = "Removes a user from a group. This command may not be used on bots or users who are not in the server.",
	...
	run = function(self, message, argString, args, guildSettings, conn)
		...
		for user in message.mentionedUsers:iter() do
			...
			if not member or not member:hasRole(role) or user.bot then
		...
		if #validUsers==0 then
			message:reply{embed={
				description = "All inputted users either were not in Group #"..groupNum..", are bots, or are not in the server.",
				fields = {
					{name = "Users not removed", value = invalidUsersString}
				},
		...
		if #invalidUsers==0 then
			message:reply{embed={
				description = "All inputted users have now been removed from Group #"..groupNum..".",
				fields = {
					{name = "Users removed", value = validUsersString}
				},
				color = discordia.Color.fromHex("00ff00").value
			}}
		else
			message:reply{embed={
				description = "Some of the inputted users have now been removed from Group #"..groupNum..". The rest either were not in Group #"..groupNum..", are bots, or are not in the server.",
				fields = {
					{name = "Users removed", value = validUsersString},
					{name = "Users not removed", value = invalidUsersString}
				},
