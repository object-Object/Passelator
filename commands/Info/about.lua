	...
	run = function(self, message, argString, args, guildSettings, conn)
		...
		message.channel:send{
			embed = {
				...
				fields = {
					{name = "Getting started", value = [[**0.** The server admins need to set up the bot using `]]..guildSettings.prefix..[[settings`. Specifically, the Group Category and Group Channel settings must be set before the groups feature may be used.
**1.** Create a new group using `]]..guildSettings.prefix..[[new <group name>`.
